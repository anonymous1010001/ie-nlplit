...
def main():
    ...
    if args.do_train:
        ...
        num_epochs=list(range(3))

        for epoch in tqdm(num_epochs):
            with strategy.scope():
                for (input_ids, input_mask, segment_ids, valid_ids, label_ids,label_mask) in dist_dataset:
                    loss = train_step(input_ids, input_mask, segment_ids, valid_ids, label_ids,label_mask)
                    loss_metric.update_state(loss)
            print('Training loss: {}'.format(round(float(loss_metric.result()), 4)))
